...
label myintro:
    ...
    player.say 'Итак начнём. Во первых мне надо ходить на работу. За каждый день, который я заполняла бумаги, мне будет еженедельно начисляться зарплата из бюджета школы.'
    player.say 'Бюджет школы пополняется ежемесячно 1 числа, в зависимости от среднего образования учеников по школе. Из него платятся деньги не только мне, но и всем учителям. К тому же могут возникать различные нужды школы по которым тоже надо платить из бюджета.'
    player.say 'Чтобы взимодействовать со школой, в моём офисе есть компьютер, там можно выставлять различные правила и закупать различные предметы обстановки.'
    player.say 'Только НЕ ТРАТЬТЕ всю сумму сразу! Мне ещё учителям платить каждую неделю!!!'
    player.say 'Можно взимодействовать с людьми напрямую через общение.'
