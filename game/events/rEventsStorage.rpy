label event_loc_storage_0_no1:
    ...
    if player.getCorr() > 30:
        'А также то, что в штанишках у неё, похоже, немного больше, чем она рассказывает. Надо будет как нибудь проверить, что она там прячет.'
        $ player.incLust(10)
        $ player.incCorr(1)
    $ move(curloc)
...
label event_loc_storage_0_no2:
    ...
    'Вы заходите в кладовку и видите, что [st1.fname] кого-то ждёт. Искренне надеясь, что она ждёт не вас, вы выходите.'
    $ move(prevloc)
...
label event_loc_storage_0_no3:
    ...
    '[st1.fname], похоже, кого-то ждёт в кладовке. Вы немного поболтали с ней и, судя по её удивлённому виду, поняли, что ждала она не вас.'
    if player.getCorr() > 30:
        'А, судя по позе, возможно хотела предаться вдали от глаз более приятным занятием, чем пустое ожидание. Не стоило с ней разговаривать. Стоило подсмотреть сначала. Но кто же знал?'
        $ player.incLust(-10)
        $ player.incCorr(-1)
    $ move(curloc)
...
label event_loc_storage_0_no4:
    ...
    '[st1.fname] подвернула ногу, пытаясь дотянуться до верхней полки со спортинвентарём. Хорошо, что вы вовремя зашли!'
    menu:
        ...
        'Не обращать внимания':    
            'Вы решили не обращать внимания на ранение ученицы. В конце концов, вы директор, а не нянька.'
            $ st1.incLoy(5)
...
label event_loc_storage_30_low1:
    ...
    'Молодые влажные киски, нежно трущиеся друг об друга. Неспешное перемешивание их соков и эротичное хлюпание. Эта картинка буквально застыла у вас в глазах и голове.'
...
label event_loc_storage_30_low2:
    ...
    menu:
        'Всё таки лучше учить хоть так, чем никак.' if player.getCorr > 40:
            hide tempPic
...
label event_loc_storage_30_low4:
    ...
    st1.say 'Я, я... Пёрышко из мата просто в трусики попало... - ученица быстро собралась и убежала, оставляя вас в раздумьях: "Откуда в войлочных матах перья?"'
...
label event_loc_storage_50_mid1:
    ...
    '[st2.fname] сильными толчками вгонял свой член в одноклассницу, стонавшую под его неистовым напором. Явно, что ученики сильно спешили отдаться друг другу, так как даже не успели снять одежду. [st1.fname] громко стонала, ощущая, как стенки её влагалища таранит молодой член парня. Парочка настолько завелась, что совершенно не обращает на вас внимания!'
    'Твёрдый ствол плавно двигался в киске, проникая своей разбухшей от возбуждения головокой всё глубже. Вы потеряли связь с реальностью, для вас существовал только этот прекрасный член, таранящий возбуждённое тело.'
    stop movie
    hide movie   
...
label event_loc_storage_50_mid2:
    ...
    'Вы даже слегка удивились, как она может принять такой пенис в свою отнюдь не титаническую щёлку. Но как-то принимала. И даже получала от этого неслабое удовольствие. Отдельной темой была [st2.fname].'
