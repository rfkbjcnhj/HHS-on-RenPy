    ...
    def getFlirtReaction(user):
        if user.getWill() > player.getCorr():
            ...
            if user.body.sex() == 'male':
                return user.name +' скептически посмотрел на вас, явно не одобряя ваше поведение. Его лояльность к вам серьёзно упала.'
            else:
                return user.name +' скептически посмотрела на вас, явно не одобряя подобное поведение. Её лояльность к вам серьёзно упала.'
        else:
            user.incLoy(1)
            user.incCorr(0.2)
