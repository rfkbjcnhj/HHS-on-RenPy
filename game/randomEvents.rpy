...
label event_loc_class2_0_1:
    ...
    'Да! И девушки встают в стойку черлидеров, представляя, что в руках у них понпоны.'
...
label event_loc_class2_0_10:
    ...
    menu:
        ...
        'Просто наказать её':
            ...
            jump scoldAll
        'Ничего не делать':
            'Вы просто хмкнули, и погладили спящую девушку по голове, не желая тревожить её сон. К сожалению парту придёться чистить за счёт школы.'
            $ st1.incLoy(10)
            $ setRep(3,5)
            $ setLoy(3,5)
            $ school.budget -= 100
...
label event_loc_class2_5_5:
    ...
    if player.getCorr() > 50:
        ...
        player.say '"Будем надеятся, что в кино на последнем ряду, да с членом любимого во рту, [st1.fname] наконец то разберёться, бойфренд ей [st2.fname], или не бойфренд."'
        if st1.getCorr() > 30:
...
label event_loc_class2_10_1:
    ...
    'Под вашим взглядом вскоре обнажились два небольших, но молодых и крепких достоинств. Их розовые головки обнажились и смотрели вверх. Прямо на вас. Вы плочувствовали как между ножек становится влажно, но не дали желанию прорваться, сейчас несколько иная задача.'
    ...
    'Долго это пордолжаться не могло и вскоре из их членов полилось густое семя, оставляя свои следы на полу, партах, и их телах.'
...
label event_loc_class2_25_2:
    ...
    'Прежде чем вы успели открыть рот, он с громким стоном спусил, заляпав школьницу своим семенем. [st2.fname] шокированно смотрит на вас, и по её грудям и животику медленно стекают капли свежей спермы.'
    menu:
        'Всех наказать':
            $ scoldWho = [st1,st2]
            jump scoldAll
...
label event_loc_class2_40_4:
    ...
    'Вы даже засмотрелись, как она ловко двигает в рукой в такт движениям головы. Да, судя по сладострастным причмокиваням, девушка получает от минета не меньшее удовольствие, чем парень! Редкий случай в наши дни. Что же с ними делать?'
    menu:
        'Всех наказать':
            $ scoldWho = [st1,st2]
            jump scoldAll
...
label event_loc_class2_25_6:
    ...
    'Довольно инетересный способ признания в любви Вы имели честь наблюдать. По крайней мере бешенный стояк парня напротив определённо намекал на ответные чувства. А [st2.fname] всё стояла перед ним со спущенными трусиками и говорила что то про то, как её киска жить без члена не может. Да, по обилию выделяющейся влаги киска просто плачет по хорошему члену.'
    menu:
...
label event_loc_class2_25_7:
    ...
    'Зайдя в класс, вы заметили вашу ученицу, самоудовлетворяющуся, кхм, столом. То ли все парни испарились, то ли это новое напрвление в ориентации. Менсафилия какая то. Вы не смогли не заметить как сквозь тонкую ткань трусиков на стол уже просочилось довольно неслабое количество смазки.'
    menu:
        'Наказать':
            $ scoldWho = [st1]
            jump scoldAll
...
label event_loc_class2_20_1danokova:
    ...
    '[st2.fname] стонет, но двигает бёдрами в такт движеням ног учительницы.'
...
label event_loc_class2_15_2danokova:
    ...
    'Вы наблюдаете за биологичкой, поучающей ученика. Встав немного сзади, она умело массирует его член, провоцируя скорое семяизвржение. Вот если бы не её заковыристый язык, цены бы ей не было!'
...
label event_loc_class2_35_4:
    ...
    st1.say ' А вон там вон киска, - говорила [st1.fname] показывая однокрассникам своё совсем уже не детское тело.'
...
label event_loc_class2_60_5:
    ...
    'Вы наблюдаете, как [st2.fname] пытается впихнуть в анус одноклассницы откровенно невпихуемое. Хотя не похоже, что [st1.fname] хоть как то этому противится. Скорее наобарот, приняла максимально расслабленную позу, и покорно ожидает пока член начнёт теребить стенки её кишки.'
    menu:
        'Наказать':
            $ scoldWho = [st1,st2]
            jump scoldAll
...
label event_loc_class2_50_9:
    ...
    menu:
        ...
        'Посмотреть':
            ...
            st1.say 'По-до-жди! - выдвавла девушка по слегу в перерывах между втрожениями члена в её лоно, - Не так рез-ко!'
...
label event_loc_class2_85_1:
    ...
    '[st2.name], ухватив за волосы другу девушку, жадно сношал её в рот. Хотя, судя по её виду, [st7.fname] совсем не против подобного обращения и с радость принимает всё, чтор может ей дать парень.'
    'А вот [st3.fname] похоже не только страстен, но и довольно силён! Шутка ли, держать килограмм 40 чистого веса на руках, да ещё и с завидным упорстовом приседать...'
    ...
    if player.getLust() > 80:
        ...
        'Вы беспорядочно подвизгивали и подмахивали совсем не в такт. Пенис входил в щёлку постоянно под разными углами и с разной скоростью. Вы почти начали терять сознание от наслаждения, когда вас настиг спасительный оргазм.'
...
label event_loc_class3_0_no6:
    ...
    '[st1.fname] внимательно читает какой-то любовный роман, машинально накручивая на палец локон спящей рядом подружки. Не желая нарушать идилию, вы тихо выходите, пока вас не заметили.'
    $ move(curloc)
...
label event_loc_class3_0_no8:
    ...
    if player.getCorr() > 70:
        ...
        player.say 'А, ну счас мы можем наблюдать типичную эякуляцию, да... - мгновенно соориентировались вы.'
...
label event_loc_class3_40_mid1:
    ...
    menu:
        ...
        'Вмешаться в процесс' if player.getCorr() > 70:
            ...
            st1.say 'Д-а-а-а!'
            st2.say 'О-о-о!'
            'Xрип кончающих парней cлился воедино, и ваши с ученицей ножки оказались забрызганы их семенем.'
...
label event_loc_class3_55_mid5:
    ...
    'Вы насилу привели в себя девушку, и выяснили, что развлекалаясь она сразу с двумя парнями. Теперь вы знаете кого наказать и за что.'
    menu:
        'Наказать их':
            $ scoldWho = [st1,st2,st3]
            jump scoldAll
...
label event_loc_class4_0_no6:
    ...
    if player.getIntel() > 50:
        ...
        'Ученики обязательно расскажут родителям, какой у них эредированный директор.'
        $ setRep(2,10)
    else:
...
label event_loc_class5_0_no3:
    ...
    'Это вы удачно зашли! Похоже, что [st1.fname] справляет свой день родения! Вы говорите ей свои дежурные поздравления, и оставляете учеников праздновать дальше.'
    $ move(curloc)
...
label event_loc_class5_0_no6:
    ...
    if player.getCorr() > 60:
        ...
        'Вы отчётливо представили, как [st1.fname], закончив свои ночные бои в Шмоте, заходит в спальню к подружке и, перевернув её попкой к верху, медленно вводит член в неподдатливое колечко сфинктера. У вас аж всё сжалось от реалистичности картинки предоставленной воображением.'
        $ player.incLust(10)
    $ move(curloc)
...
label event_loc_class5_10_no7:
    ...
    menu:
        ...
        'Подрядить ученика на это дело':
            ...
            'Парень, постоянно подтягивая спадающие треники, кивает вам, и бросается на помощь однокласснице. Вы, с удовлетворённым видом отворачиваетесь, как внезапно слишите глухой удар и крик девушки за спиной.'
...
label event_loc_beach_5_24:
    ...
    menu:
        'Сделать ей замечание':
            'Вы подходите к учительнице, и просите её надеть топ, потому что все смотрят, что скажется на репутиции школы. [st1.fname] извиняется, и начинает в спешке натягивать лифчик. Удивительно, как такая скромная учительница умудрилась раздеться до трусов перед публикой.'
            $ st1.incLoy(-10)
            $ setRep(5,5)
            $ st1.incCorr(-5)
        'Не обращать внимания':
...
label event_loc_beach_50_5:
    ...
    'Вы обратили внимание на аномально высокую концентрацию ног в пляжной раздевалке. Аккуратно заглянув, вы увидели, как [st2.fname] посасывает грудь девушки с паралелльного класса. Его член призывно тёрся о небритый лобок девушки намекая на более личное знакомство.'
...
label event_loc_beach_70_2:
    ...
    '[st2.fname] яростно сношал свою подругу, совершенно не обращая внимания на то, что за ним наблюдают десятки глаз, и даже пара камер. Пожалуй от этого он возбуждался ещё сильнее. И его крепкий член всё актичнее вторгался в пещерку школьницы.'
