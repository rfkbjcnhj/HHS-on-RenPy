...
label event_loc_class2_85_1:
    ...
    if player.getLust() > 80:
        ...
        'Вы попросили отпустить вас, после чего привели себя в порядок и попросили учеников прибрать за собой после оргии.'
        $ hadSex(st1,player)
        $ hadSex(st1,player)
        $ st1.incLoy(20)
        $ player.setLust(0)
