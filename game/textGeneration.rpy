    ...
    def reactionGen(char):
        ...
        if char in detentions and hour >= 15: # если наказан
            if school.detention == 'education':
                reaction += '[lname] сидит за партой и занимается на дополнительных занятиях. Не сильно похоже, чтобы он был в восторге от этого.'

            elif school.detention == 'upskirt' and char.getSex() != 'male':
                reaction += '[name] стоит наклонившись за партой, позволяя любому посмотреть, что у неё под юбкой. '
                if char.getLust() > 50:
                    reaction += 'Вы видите, что ей это похоже нравится. По крайней мере на трусиках отчётливо проступает влажное пятнышко.'
                    
            elif school.detention == 'cleaning':
                reaction += '[lname] стоит и медленно теребит тряпкой пол. Именно теребит, так как мытьём это можно назвать лишь с большой натяжкой. Ну чтож, на то оно и наказание.'
                
            elif school.detention == 'streetCleaning':
                if char.getSex() == 'male':
                    reaction += '[name] нехотя подметает улицы. По опущенному взгляду можно точно определить что он думает о вас, о ваших нововведениях и школе, которая заставляет его выполнять столь грязную работу. Но деньги не пахнут, не так ли?'
                else:
                    reaction += '[name] нехотя подметает улицы. По опущенному взгляду можно точно определить что она думает о вас, о ваших нововведениях и школе, которая заставляет её выполнять столь грязную работу. Но деньги не пахнут, не так ли?'
            else:
            
                if char.getSex() == 'male':
                    reaction += 'В обычной школе [name] уже драил бы полы или зубрил логарифмическую линейку, но вы решили никак не наказывать нерадивых учеников.'
                else:
                    reaction += 'В обычной школе [name] уже драила бы полы или зубрила логарифмическую линейку, но вы решили никак не наказывать нерадивых учеников.'
        ...
        if player.isSperm() == 2: # Если видна сперма
            if char.getCorr() < 25 or char.getWill() > 80:
                reaction += 'Вы замечаете, что [fname] как-то странно поглядывает на Вас. Похоже эти эти белые пятна привлекают лишнее внимание. '
                if char.getLoy() < 50:
                    reaction += 'И так как лояльность ученика не очень высока, он наверняка проговориться своим родителям.'
                    char.setRep(-0.5)
            else:
                reaction += 'Вы замечаете, что [fname] c какой-то странной улыбкой поглядывает на вас. Похоже эти белые пятна привлекают лишнее внимание. Хотя не видно, чтобы это хоть как то смущало собеседника.'
            reaction += '\n'
        
        if player.getDirty() > 1: # Если грязь
            if player.getDirty() > 4:
                reaction += '[fname] ужасно кривится от вашего запаха. Похоже кого то сейчас стошнит.'
            else:
                reaction += '[fname] кривится от вашего запаха.'
            char.setLoy(-1*player.getDirty())
            reaction += '\n'
            
        if player.getClothPurpose('usual'): # Если обыная одежда
            if char.getCorr() > 80:
                if char.getWill() < 80:
                    if char.getSex() == 'male':
                        reaction += 'Разглядывая вашу обычную одежду [name] явно скучает. Наверняка он надеялся увидеть что то более сексуальное и обтягивающее. '
                    else:
                        reaction += '[name] явно не одобряет ваш слишком строгий наряд. Она то сама уже без стеснения носит миниюбки. '
                    char.sayCount = char.sayCount/2
                else:
                    if char.getSex() == 'male':
                        reaction += '[name] вежливо кивает головой, разглядывая вашу строгую одежду. Он стал лучше к вам относиться из за того, что Вы не идёте на поводу у толпы и не снимаете с себя шмотки, улюлюкая при этом.'
                    else:
                        reaction += '[name] мило улыбается и поправляет свою одежду, разглядывая вашу. Она стала лучше к вам относиться из за того, что вы плевать хотели на последние тренды. '
                    char.setLoy(1)
            else:
                if char.getSex() == 'male':
                    reaction += '[name] вежливо здоровается с вами и настраивается на серьёзный разговор, к которому располагает ваша, довольно строгая одежда. '
                else:
                    reaction += '[name] кивает Вам и и настраивается на серьёзный разговор, к которому располагает ваша, довольно строгая одежда. '
                if char.getLoy() > 500:
                    temp = 'он'
                    if char.getSex() != 'male': temp = 'она'
                    reaction += 'При следующем разговоре с родителями, [temp] наверняка упомянёт вашу строгость'
            reaction += '\n'
